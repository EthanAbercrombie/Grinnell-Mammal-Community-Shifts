...
---
This code is borrowed and inspired by Adam Smith and the "Best Practices in Species Distribution Modeling" Workshop series.
...
load('C:/Grinnell-Mammal-Community-Shifts/Code/Occurence Data Cleaning/GADM Canada, USA, Mexico Level 2 WGS84.rda')
  ...
  #It can take a long time for my PC to generate plots using Raster's plot function on a spatialpolygondataframe. To remedy this I will convert the spatial polygon objects to standard data frames. GGplot can be then used to evaluate spatial relationships.
	#states
  ...
  #countiesDF <- merge(counties_for, states@data, by = "id")
...
ggplot(data = statesDF, aes(x = long, y = lat, group = group)) +
  geom_polygon() +
  geom_point(data = filter(speciesNoNas, coordinateUncertaintyInMeters) , 
...
Filter data. Remove all occurences with uncertaintity > 100m. Remove all occurences collected before 1970.
...
species <- species %>% 
  filter(year >= 1970) %>% 
  filter(coordinateUncertaintyInMeters <= 1000) #Considering climate changes rapidly in montane environments, I am going to remove occurence that have uncertainty greater than 1000m.
...
ggplot(data = statesDF, aes(x = long, y = lat, group = group)) +
  geom_polygon() +
  geom_point(data = filter(speciesNoNas, coordinateUncertaintyInMeters != "NA") , 
             aes(x = decimalLongitude, 
             y = decimalLatitude, 
             color = coordinateUncertaintyInMeters)) +
