---
title: "Dipodomys_agilis - occurrence data cleaning"
...
species <- read.delim('C:/Grinnell-Mammal-Community-Shifts/Data/Species Occurrence Data/Dipodomys agilis/GBIF/occurrence.txt')
...
rangeMap <- st_read('C:/Grinnell-Mammal-Community-Shifts/Data/IUCN Species Ranges/Species Ranges_Exports_QGIS/Dipodomys_agilis/Dipodomys_agilis.shp')
```
...
#Evaluate the number of observations with 'Issue'
            ...
            grepl(c('TAXON_MATCH_HIGHERRANK'),
                  issue
            ))
fi$gbifID
...
#             "C:/Grinnell-Mammal-Community-Shifts/Data/Species Climate Data/Dipodomys_agilis_climate",
#             format = 'GTiff',
            # overwrite = T)

#Add path to tif file
climate_data <- stack('C:/Grinnell-Mammal-Community-Shifts/Data/Species Climate Data/Dipodomys_agilis_climate.tif')
```
...
hist(speciesSf_filtered$Dipodomys_agilis_climate.12)
min(speciesSf_filtered$Dipodomys_agilis_climate.12)

#0 is excluded.
speciesSf_filtered <- st_as_sf(speciesSf_filtered) %>% 
  mutate(Annual_Temp = Dipodomys_agilis_climate.1 / 10,
         Annual_Precip = Dipodomys_agilis_climate.12 / 1000) %>% 
  filter(Annual_Temp <= quantile(Annual_Temp, 0.95) &
           Annual_Precip <= quantile(Annual_Precip, 0.95))
```

Evaluate occurrences outside of range map buffer that have recorded coordinate uncertainty.

```{r}
require(ggsflabel)

speciesSf_filtered_locality <- speciesSf_filtered %>% 
  mutate(occurrence_number = 1:nrow(speciesSf_filtered)) %>% 
  filter(coordinateUncertaintyInMeters != "NA" &
           within_buffer == 0) %>% 
...
ggplot() +
  ...
  geom_sf(data = states,
               fill = NA,
               color = "darkred") +
  geom_sf(data = speciesSf_filtered_locality,
             aes(color = gbifID),
          inherit.aes = FALSE) +
  geom_sf_label_repel(data = speciesSf_filtered_locality,
                aes(label = occurrence_number),
                    force = 50,
                max.overlaps = 30)+
  coord_sf(xlim = c(-124, -108), 
           ylim = c(23,35), 
           expand = TRUE) +
...
unique(speciesSf_filtered_locality$locality)

#occurrence 18
View(speciesSf_filtered_locality %>% 
       filter(occurrence_number == 18))
#Notes: All looks OK. Taxon match higher rank warning. Collected by Troy L. Best.

#occurrence 85
View(speciesSf_filtered_locality %>% 
       filter(occurrence_number == 85))
#Notes: All looks OK. Taxon match higher rank warning

#occurrence 86
View(speciesSf_filtered_locality %>% 
       filter(occurrence_number == 86))

#Notes: All looks OK. Taxon match higher rank warning
#occurrence 87
View(speciesSf_filtered_locality %>% 
       filter(occurrence_number == 87))
#Notes: All looks OK. Taxon match higher rank warning
#Occurrence 22
  
#row21
#"3 km SE of Colonia Vincente Guerrero"

# row22
#"3 km SE of Colonia Vincente Guerrero"

# row23
#"13 km SW of Santa Rita"
...
# row104
#"4 mi E Ranchita on County Hwy. S22"

# row105
#"4 mi E Ranchita on County Hwy. S22"

# row131
#"San Felipe Valley, 9.5 mi S San Felipe"

# row132
#"Echo Valley, 5 mi N Descanso"
