...
pfa_data <- pfa_data %>% 
  bind_rows(filter(pfa_data,
                   Era == 'M')) %>%  
  mutate(Era = if_else(duplicated(Era) &
                         duplicated(Region),
                       "HM_conservative",
                       Era)) #create hypothetical group that is conservative with respect to HM. Extends the range above and below a site.
...
site_data <- read_csv('/Users/ethanabercrombie/Library/CloudStorage/Box-Box/Grinnell-Mammal-Community-Shifts/Rowe et al. 2017 - FINAL Occupancy Analyses/R Scripts and Mark Data/PSI-by-Site-08mar2011.csv') %>% 
  ...
  mutate(site_name = if_else(duplicated(site_name),
                             ...
                             site_name)) %>% 
  bind_rows(filter(site_data,
                   Era == 'H')) %>% #Make duplicates of historical site, and rename Era to "HM_conservative". This will be used to calculate conservative contemporary CCIs for historical sites.
  mutate(Era = if_else(duplicated(site_name) &
                         duplicated(Era),
                       "HM_conservative",
                       Era)) %>% 
